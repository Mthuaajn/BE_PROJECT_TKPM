...
import { IFileExtensionComicsPlugin } from '~/models/FileExtensionComicsManagement/IFileExtensionComicsPlugin';
  ...
  async (req: Request<ParamsDictionary, any>, res: Response, next: NextFunction) => {
    ...
    if (source != null) {
      ...
      if (plugin != null) {
        ...
        const result: ContentStoryComic | null = await plugin.contentStory(title, chap);
        ...
        if (result != null) {
          ...
          const fileExtensionPlugin: IFileExtensionComicsPlugin | null =
            fileExtensionManager.select(`${fileExtensionType}ComicsPlugin`);
          const fileTxtExtensionPlugin: IFileExtensionComicsPlugin | null =
            fileExtensionManager.select('HTMLComicsPlugin');
